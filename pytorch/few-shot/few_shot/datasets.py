...
class OmniglotDataset(Dataset):
    def __init__(self, subset):
        ...
        # columns: ['subset', 'alphabet', 'class_name', 'filepath', 'id', 'class_id']
...
class Fabric(Dataset):
    def __init__(self, subset):
        """Dataset class representing Fabric dataset
        ...
        self.mean_std_path = DATA_PATH + '/Fabric/mean_std_value_train.pkl'
        self.mean, self.std = self.get_mean_std(self.mean_std_path)
        ...
        self.transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(mean=self.mean[0],
                                 std=self.std[0])
        ])
    ...
    def index_subset(subset):
        ...
        # Quick first pass to find total for tqdm bar
        subset_len = 0
        for root, folders, files in os.walk(DATA_PATH + '/Fabric/images_{}/'.format(subset)):
            subset_len += len([f for f in files if f.endswith('.bmp')])
