...
from few_shot.train import gradient_step
...
class NShotTaskSampler(Sampler):
    def __init__(self,
        ...
        # Arguments
            ...
            n_shot: int. Number of support samples for each class in the n-shot classification tasks.
            k_way: int. Number of classes in the n-shot classification tasks.
            q_queries: int. Number of query samples for each class in the n-shot classification tasks.
            num_tasks: Number of n-shot tasks to group into a single batch
            fixed_tasks: If this argument is specified this Sampler will always generate tasks from the specified classes
        """
...
class TestFewShot(Callback):
    """Test a network on an n-shot, k-way classification tasks after all epochs.

    # Arguments
        checkpoint_filepath: file path of the best model checkpoint.
        eval_fn: Callable to perform few-shot classification. Examples include `proto_net_episode`,
            `matching_net_episode` and `meta_gradient_step` (MAML).
        num_tasks: int. Number of n-shot classification tasks to evaluate the model with.
        n_shot: int. Number of samples for each class in the n-shot classification tasks.
        k_way: int. Number of classes in the n-shot classification tasks.
        q_queries: int. Number query samples for each class in the n-shot classification tasks.
        taskloader: Instance of NShotWrapper class
        prepare_batch: function. The preprocessing function to apply to samples from the dataset.
        prefix: str. Prefix to identify dataset.
    """
    ...
    def __init__(self,
                 ...
                 prefix: str = 'test_',
                 **kwargs):
        super(TestFewShot, self).__init__()
...
class EvaluateClassifier(Callback):
    """Evaluate a network on an n-shot, k-way classification tasks after every epoch.
    ...
    def __init__(self,
                 ...
                 prefix: str = 'val_',
                 **kwargs):
        super(EvaluateClassifier, self).__init__()
        self.eval_fn = eval_fn
        self.taskloader = taskloader
        self.prepare_batch = prepare_batch
        self.prefix = prefix
        self.kwargs = kwargs
        self.metric_name = f'{self.prefix}acc'
...
def prepare_classifier_task() -> Callable:
    """Typical n-shot task preprocessing.

    # Returns
        prepare_classifier_task_: A Callable
    """
    def prepare_classifier_task_(batch: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        """move to GPU."""
        x, y = batch
        x = x.double().cuda()
        # Create dummy 0-(num_classes - 1) label
    ...
    return prepare_classifier_task_
